<div class="sidebar-actions">
    <h6>Acciones Rápidas</h6>
    <div class="action-buttons">
        <a href="<?php echo e(route('admin.mascotas.create')); ?>" class="btn btn-primary btn-sm">
            <i class="fas fa-plus"></i> Nueva Mascota
        </a>
        <a href="<?php echo e(route('reportes.create')); ?>" class="btn btn-warning btn-sm">
            <i class="fas fa-flag"></i> Nuevo Reporte
        </a>
    </div>
</div><?php /**PATH C:\Users\Juanda\Desktop\Rescatando-mascotas-foreve\resources\views/portals/admin/partials/sidebar/quick-actions.blade.php ENDPATH**/ ?>